...
static int		get_type_of_token_2(int next_state, int last_state)
{
	if (last_state == LEFT_REDIRECTION_STATE
		...
		return (IN_RDIR_TYPE);
	else if (last_state == RIGHT_REDIRECTION_STATE
		|| next_state == RIGHT_REDIRECTION_STATE)
		return (OUT_RDIR_TYPE);
	else if (last_state == DOUBLE_LEFT_REDIRECTION_STATE
		|| next_state == DOUBLE_LEFT_REDIRECTION_STATE)
		return (HEREDOC_TYPE);
	else if (last_state == DOUBLE_RIGHT_REDIRECTION_STATE
...
int				get_type_of_token(int next_state, int last_state)
{
	...
	else if (last_state == RIGHT_AGGREGATION_V_STATE
